...
**/
    ...
    if (found) {
        ...
        tBTNodePtr toBeAdded = malloc(sizeof(struct tBTNode));
	if (toBeAdded == NULL) return;
        toBeAdded->Cont = Content;
        toBeAdded->LPtr = toBeAdded->RPtr = NULL;

        if (where == NULL) {
            (*RootPtr) = toBeAdded;
        } else if(where->Cont > Content) {
            where->LPtr = toBeAdded;
        } else {
            where->RPtr = toBeAdded;
        }
    }
}
...
**/
    if (RootPtr) {
        tStackP stackPoint;
        SInitP(&stackPoint);
        Leftmost_Preorder(RootPtr, &stackPoint);
        tBTNodePtr tmp;
        while (!SEmptyP(&stackPoint)) {
            tmp = STopPopP(&stackPoint);
            Leftmost_Preorder(tmp->RPtr, &stackPoint);
        }
    }
}
...
**/
    if (RootPtr) {
        tStackP stackPoint;
        SInitP(&stackPoint);
        Leftmost_Inorder(RootPtr, &stackPoint);
        tBTNodePtr tmp;
        while (!SEmptyP(&stackPoint)) {
            tmp = STopPopP(&stackPoint);
            BTWorkOut(tmp);
            Leftmost_Inorder(tmp->RPtr, &stackPoint);
        }
    }
}
...
**/
    if (RootPtr) {
        ...
        SInitP(&stackPoint);
        SInitB(&stackBool);
        Leftmost_Postorder(RootPtr, &stackPoint, &stackBool);
        tBTNodePtr tmp;
        int left;
        while (!SEmptyP(&stackPoint)) {
            tmp = STopPopP(&stackPoint);
            SPushP(&stackPoint, tmp);
            left = STopPopB(&stackBool);
            if (left) {
                SPushB(&stackBool, FALSE);
                Leftmost_Postorder(tmp->RPtr, &stackPoint, &stackBool);
            } else {
                STopPopP(&stackPoint);
                BTWorkOut(tmp);
            }
...
**/
    if ((*RootPtr)) {
        ...
        do {
            if ((*RootPtr) == NULL) {
                if (!SEmptyP(&stackPoint)) {
                    (*RootPtr) = STopPopP(&stackPoint);
                }
            } else {
                if ((*RootPtr)->RPtr != NULL) {
                    SPushP(&stackPoint, (*RootPtr)->RPtr);
                }
                tmp = *RootPtr;
        ...
        } while (((*RootPtr) != NULL) || (!SEmptyP(&stackPoint)));
    }
}
